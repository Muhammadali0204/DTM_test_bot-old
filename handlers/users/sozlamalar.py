...
async def sozlama_back(msg : types.Message, state : FSMContext):
    await msg.answer("<b>Yangi ism-familiyani yuboring : </b>", reply_markup=ortga.ortga)
...
async def jdei(msg : types.Message):
    await msg.delete()
    await msg.answer("<b>Quyidagi tugmalardan foydalaning 👇</b>", reply_markup=sozlama.sozlama)
...
async def ismi(msg : types.Message, state : FSMContext):
    if all(x.isalpha() or  belgi(x) for x in msg.text):
        if len(msg.text) > 5 and len(msg.text) < 31:
            ...
            await msg.answer("<b>Ismingiz muvaffaqiyatli o'zgartirildi ✅</b>", reply_markup=menu.menu)
            await state.finish()
        else:
            await msg.answer("<b>Ism-familiya uzunligi 5 ta belgidan ko'p va 30 ta belgidan kam bo'lmasligi lozim !</b>\n\n<i>Qayta kiriting : </i>", reply_markup=ortga.ortga)
    else:
        await msg.answer("<b>Faqatgina lotin harflari, bo'sh joy va <code>' `</code> shu kabi belgilardan foydalanishingiz mumkin !</b>\n\n<i>Qayta kiriting : </i>", reply_markup=ortga.ortga)
