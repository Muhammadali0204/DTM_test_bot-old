...
async def test(msg : types.Message, state : FSMContext):
    await msg.answer("Qo`shmoqchi bo`lgan fan nomini kiriting : (Asosiy fan uchun)", reply_markup=ortga.ortga)
    await state.set_state("add fan1 asosiy")
    

@dp.message_handler(text="📚Blok test (5 ta fan)", state="admin test turi1", chat_id = ADMINS)
async def test(msg : types.Message, state : FSMContext):
    await msg.answer("Qo`shmoqchi bo`lgan fan nomini kiriting : (Blok test uchun) ", reply_markup=ortga.ortga)
...
async def add_asosiy_fan(msg : types.Message, state : FSMContext):
    try :
        ...
        await msg.answer("<b>Muvaffaqiyatli qo'shildi ✅</b>", reply_markup=admin_menu.menu)
        await state.finish()
    except:
