...
async def test(msg : types.Message, state : FSMContext):
    ...
    await msg.answer("Qaysi fan uchun qo'shmoqchisiz : ", reply_markup=fanlar_keyboard.fanlar(data))
...
async def test(msg: types.Message, state: FSMContext):
    await msg.answer("Test faylini va javoblarni caption da yuboring : ", reply_markup=ortga.ortga)
...
async def jhsijfh(call : types.CallbackQuery, state : FSMContext):
    ...
    await call.message.answer("Test faylini va javoblarini caption da yuboring : ", reply_markup=ortga.ortga)
...
@dp.message_handler(state="add test asosiy fayl", content_types=types.ContentTypes.DOCUMENT)
...
@dp.message_handler(state="add test blok fayl", content_types=types.ContentTypes.DOCUMENT)
async def kjnheikvnb(msg : types.Message, state : FSMContext):
...
@dp.message_handler(state="add test majburiy fayl", content_types=types.ContentTypes.DOCUMENT)
async def kjnheikvnb(msg : types.Message, state : FSMContext):
...
async def ihevif(msg : types.Message, state : FSMContext):
    db_tests.add_test(temp[f"{msg.from_user.id} asosiy file_id"], temp[f"{msg.from_user.id} add test asosiy fan_id"],temp[f"{msg.from_user.id} asosiy javoblar"], msg.html_text)
...
async def ihevif(msg : types.Message, state : FSMContext):
    db_tests.add_test(temp[f"{msg.from_user.id} blok file_id"], temp[f"{msg.from_user.id} add test blok fan_id"],temp[f"{msg.from_user.id} blok javoblar"], msg.html_text)
...
async def ihevif(msg : types.Message, state : FSMContext):
    db_tests.add_test(temp["majburiy file_id"], 0,temp[f"{msg.from_user.id} majburiy javoblar"], msg.html_text)
